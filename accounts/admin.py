...
class MemberProfileInline(admin.TabularInline):
    ...
    extra = 0
    fields = (
        'farm_size', 'trees_planted', 'conservation_practices',
...
class CustomUserAdmin(UserAdmin):
    ...
    list_filter = (
        'user_type', 'membership_type', 'is_verified', 'is_active',
        'is_staff', 'county', 'gender', 'education_level'
    )
    search_fields = (
        'username', 'email', 'first_name', 'last_name', 'phone', 'id_number'
    )
    ...
    fieldsets = (
        ...
        ('Location Information', {
            'fields': ('county', 'sub_county', 'ward', 'village', 'postal_address', 'postal_code')
        }),
        ('Membership Information', {
            'fields': (
                'user_type', 'membership_type', 'organization_name',
...
class MemberProfileAdmin(admin.ModelAdmin):
    list_display = (
        'user', 'farm_size', 'trees_planted', 'events_attended',
    ...
    fieldsets = (
        ('Member', {'fields': ('user',)}),
        ('Agricultural Information', {'fields': ('farm_size', 'trees_planted', 'conservation_practices')}),
        ('Activity Tracking', {'fields': ('total_donations', 'events_attended', 'training_completed')}),
        ('Contact Preferences', {'fields': ('receive_newsletter', 'receive_sms', 'receive_email')}),
...
class ExecutiveCommitteeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('user', 'position', 'order', 'is_active')}),
        ('Term Information', {'fields': ('term_start', 'term_end', 'responsibilities')}),
        ('Special Permissions', {
